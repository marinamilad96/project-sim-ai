...
class generate_xml_files:
    ...
    def create_xml(self):
        ...
        for i, self.row in enumerate(arr):
            self.paramters, self.InfectionRate, self.IncubationRate, self.RecoveryRate, self.SamplingRate, self.S, self.E, self.I, self.R, self.Sample, self.InfectionRate_M,  self.MigrationRate_0to1, self.MigrationRate_1to0 = self.row
            ...
            print(f"Run {i+1}: paramters={self.paramters}, InfectionRate={self.InfectionRate}, "
                  ...
                  f"SamplingRate={self.SamplingRate}, S=str({self.S}), E=str({self.E}), I={self.I}, R={self.R}, Sample={self.Sample}")
            ...
            ET.SubElement(trajectory, "samplePopulation",
                          spec="RealParameter", id="R", value=str(self.R), dimension="2")
            ET.SubElement(trajectory, "samplePopulation",
            ...
            reaction2 = ET.SubElement(
                plate, "reaction", spec="Reaction", rate=str(self.RecoveryRate))
            reaction2.text = "I[$(i)] -> R[$(i)]"
            ...
            reaction4.text = "I[0]:1 -> R[0] + sample[0]:1"

            # Outside infection
            reaction5 = ET.SubElement(
                trajectory, "reaction", spec="Reaction", rate=str(self.InfectionRate_M))
            reaction5.text = "S[1] + I[1]:1 -> I[1]:1 + E[1]:1"
