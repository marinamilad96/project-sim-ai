    ...
    def create_xml(self):
        ...
        for i, self.row in enumerate(arr):
            ...
            simulate = ET.SubElement(
                run, "simulate", spec=self.config['run']["simulate"]["spec"], id="tree")
            trajectory = ET.SubElement(
                simulate, "trajectory", spec="StochasticTrajectory", id="traj",  endsWhen="sample==100000")

            # Add population elements
            ET.SubElement(trajectory, "population",
                          spec="RealParameter", id="S", value=str(self.S))
            ET.SubElement(trajectory, "population",
                          spec="RealParameter", id="E", value=str(self.E))
            ET.SubElement(trajectory, "population",
                          spec="RealParameter", id="I", value=str(self.I))

            # Add samplePopulation elements
            ET.SubElement(trajectory, "samplePopulation",
                          spec="RealParameter", id="R", value=str(self.R))
            ET.SubElement(trajectory, "samplePopulation",
                          spec="RealParameter", id="sample", value=str(self.Sample))

            # Add reaction Migrationelements
            ET.SubElement(trajectory, "reaction", spec="Reaction", rate=str(
                self.InfectionRate)).text = "I + S -> I + E"
            ET.SubElement(trajectory, "reaction", spec="Reaction",
                          rate=str(self.IncubationRate)).text = "E -> I"
            ET.SubElement(trajectory, "reaction", spec="Reaction",
                          rate=str(self.RecoveryRate)).text = "I -> R"
            ET.SubElement(trajectory, "reaction", spec="Reaction", rate=str(
                self.SamplingRate)).text = "I -> R + sample"

            # Add logger elements
            logger1 = ET.SubElement(
                run, "logger", spec="Logger", fileName="$(filebase).traj", logEvery=str(self.logEvery))
            ET.SubElement(logger1, "log", idref="traj")

            logger2 = ET.SubElement(run, "logger", spec="Logger", mode="tree",
                                    fileName="$(filebase).full.trees", logEvery=str(self.logEvery))
            log2 = ET.SubElement(logger2, "log", spec="TypedTreeLogger")
            ET.SubElement(log2, "tree", spec="PrunedTree",
                          samplePops="R", simulatedTree="@tree")

            logger3 = ET.SubElement(run, "logger", spec="Logger", mode="tree",
                                    fileName="$(filebase).sampled.trees", logEvery=str(self.logEvery))
            ...
            # Write to XML file
            tree.write(self.config["FilePath"]["outputFilePath"] +
                       f"{self.paramters}_{i+1}.xml", encoding="utf-8", xml_declaration=True)
