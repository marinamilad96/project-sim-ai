...
class ManipulateSeqFiles:
    ...
    def create_df_seq_file(self):
        ...
        self.df_seq_file['FileName'] = self.df_seq_file['FileName'].astype(str)
        self.df_seq_file = self.df_seq_file[~self.df_seq_file['FileName'].str.startswith('._')]
    ...
    def map_to_key(self, description):
        ...
        for key, values in self.cfg['extraction_abbreviation'].items():
            if any(value in description for value in values):
                return key
        for key, values in self.cfg['Other_Abbreviations'].items():
            if any(value in description for value in values):
                return key
        return 'Other'
...
class ExtractSeqToExcel:
    ...
    def apply_filter(self):
        ...
        self.post_remained_rows = self.remained_rows[~self.remained_rows['ID_xsl_prefix'].isin(filtered_ids)]
        self.post_remained_rows.to_csv(self.cfg['Extract_folder']['de_post_remained_rows'], index=False, encoding='utf-8-sig')
        return 'Filtered, remained, and post-remained DataFrames created successfully'
    
    def create_df_duplicated_ID_final(self, df: pd.DataFrame, df_name: str):
        if df is None:
            raise ValueError("DataFrame not created. Call crdf_xsl_de_filtered() first.")
            
        self.df_duplicates = df[df['ID_seq_prefix'].duplicated(keep=False)]
        self.df_matched = self.df_duplicates.pivot_table(index=['ID_seq_prefix'], aggfunc='size')

        self.df_matched_rep_ID = self.df_matched.to_frame(name='amount').reset_index()
        self.df_matched_rep_ID = self.df_matched_rep_ID.sort_values('amount', ascending=False)

        output_file = f"{self.cfg['Extract_folder']['rep_IDs_file']}{df_name}.csv"
        self.df_matched_rep_ID.to_csv(output_file)
        return 'Dataframe for replicated ID created successfully'
    
    def apply_seq_to_excel(self):
        self.extract_seq_to_excel()
        self.apply_filter()
        self.duplicated_de_post_remained_ID = self.create_df_duplicated_ID_final(self.post_remained_rows, 'duplicated_de_post_remained_ID')
        self.duplicated_de_filtered_ID = self.create_df_duplicated_ID_final(self.df_xsl_de_filtered, 'duplicated_de_filtered_ID')
        return 'Filtered and other DataFrames created successfully'
